...
const ExclusiveCommunityGrid = ({propertyCategory}) => {
  let publicUrl="/"
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 12;
  const pageCount = Math.ceil(propertyCategory?.length / itemsPerPage);
  ...
  const dispatch=useDispatch()
    ...
    const paginatedPropertyTypes = propertyCategory?.slice(
      offset,
      offset + itemsPerPage
    );
  return (
...
export default ExclusiveCommunityGrid