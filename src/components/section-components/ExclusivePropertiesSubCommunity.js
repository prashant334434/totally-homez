...
import { capitalizeWords, getPropertiesSubCommunity, getSubCommunitiesPropertiesUtils, replaceHyphensAndCapitalize } from '../../utils/propertyUtils';
...
const [getLevelsInASubCommunity, setGetLevelsInASubCommunity] = useState([])
    const { property_for, property_city, property_community, property_type, property_subcomm } = useParams()
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const data = await getSubCommunitiesPropertiesUtils("dubai", property_type, property_for, orignalSubCommunity,orignalCommunity);
