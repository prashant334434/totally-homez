...
const PenthouseProductGrid = (props) => {
  ...
  const { penthouseCategoryProperties } = useSelector(
    (state) => state.penthouseCategoryProperties
  );
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getPenthouseCatgoryProperties());
  }, [dispatch]);

  if (penthouseCategoryProperties?.length > 0) {
    carouselItems = penthouseCategoryProperties;
  }
  const url = (titleName) => {
  ...
  return (
    <div>
      <div>
        <div id="penthouseRef" tabindex="2" className="ltn__product-slider-area ltn__product-gutter pt-40  plr--7">
...
export default PenthouseProductGrid;
