...
const SubCommunityExclusivePropertyGrid = ({propertySubCategory}) => {
    const history=useHistory()
    let publicUrl="/"
    const dispatch=useDispatch()
    const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 12;
  const pageCount = Math.ceil(propertySubCategory?.length / itemsPerPage);
  ...
  const paginatedPropertyTypes =propertySubCategory?.slice(
    offset,
    offset + itemsPerPage
  );
  ...
  return (
    <div>
    <div>
      <div className="ltn__product-area ltn__product-gutter mb-80 ">
...
export default SubCommunityExclusivePropertyGrid