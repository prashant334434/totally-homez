...
import {
  ...
  getPenthouseCatgoryRentProperties,
...
const PenthouseProductGridRent = (props) => {
  ...
  const { penthouseCategoryRentProperties } = useSelector(
    (state) => state.penthouseCategoryRentProperties
  );
  ...
  if (penthouseCategoryRentProperties?.length > 0) {
    carouselItems = penthouseCategoryRentProperties;
  }
  const url = (titleName) => {
